...
function tauriEventMiddleware<T>(
  storeName: string,
    ...
    config(
      (partial, replace) => {
        ...
        for (const key of Object.keys(updatedState) as Array<keyof T>) {
          // Skip actions, runtime-only draw flags, and unchanged values.
          // `enabled` / `clickMode` are owned by rust; syncing them re-opens draw mode.
          if (
            typeof updatedState[key] === "function" ||
            key === "enabled" ||
            key === "clickMode" ||
            !oldState ||
            !updatedState ||
            updatedState[key] === oldState[key]
          ) {
            continue;
          }
            ...
            const objectChanged =
              ...
              (Object.keys(newObject).length !==
                ...
                Object.keys(newObject).some(
                  (nestedKey) =>
                    newObject[nestedKey as keyof typeof newObject] !==
                    oldObject?.[nestedKey as keyof typeof oldObject],
                ));
