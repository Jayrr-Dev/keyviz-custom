...
/** Drop a still-"pressed" key after this many linger periods. Catches swallowed key-ups. */
const STUCK_LINGER_MULTIPLIER = 2;
...
const createKeyEventStore = createSyncedStore<KeyEventStore>(
  ...
  (set, get) => ({
    ...
    onKeyPress(event: RawKeyEvent) {
      ...
      const existingKey =
        last >= 0
          ? groups[last].keys.find((gKey) => gKey.name === key.name)
          : undefined;
      if (existingKey) {
      ...
      // 3. add to group
        ...
        // key combination
          ...
          if (
            ...
            groups[last].keys.some((gKey) => !gKey.in(pressedKeys))
          ) {
            // history mode, partial combination, add new group
            const groupKeys = groups[last].keys.filter((gKey) =>
              gKey.in(pressedKeys),
            );
            groupKeys.push(key);
    ...
    onKeyRelease(event: RawKeyEvent) {
      ...
      const pressedKeys = state.pressedKeys.filter(
        (keyName) => keyName !== event.name,
      );
      ...
      const kIndex =
        last >= 0
          ? groups[last].keys.findIndex((key) => key.name === event.name)
          : -1;
      if (kIndex >= 0) {
    ...
    onMouseMove(event: MouseMoveEvent) {
      ...
      if (mouse.dragStart && !mouse.dragging) {
        ...
        if (dragDistance > state.dragThreshold) {
          ...
          const pressedKeys = state.pressedKeys.filter(
            (keyName) => keyName !== state.pressedMouseButton?.toString(),
          );
          ...
          const last = groups.length - 1;
          if (last >= 0) {
            groups[last].keys = groups[last].keys.filter(
              (key) => key.name !== state.pressedMouseButton?.toString(),
            );
          }
    ...
    onMouseWheel(event: MouseWheelEvent) {
      ...
      const previous_raw_key =
        state.mouse.wheel > 0 ? RawKey.ScrollUp : RawKey.ScrollDown;
    ...
    tick() {
      ...
      if (
        state.mouse.lastScrollAt &&
        now - state.mouse.lastScrollAt > SCROLL_LINGER_MS
      ) {
        if (state.pressedKeys.includes(RawKey.ScrollUp)) {
          state.onKeyRelease({
            type: "KeyEvent",
            name: RawKey.ScrollUp,
            pressed: false,
          });
        }
      ...
      const stuckAfterMs = state.lingerDurationMs * STUCK_LINGER_MULTIPLIER;
      const stuckNames: string[] = [];

      // remove keys that have exceeded linger duration
      for (const group of state.groups) {
        const updatedKeys = group.keys.filter((key) => {
          const age = now - key.lastPressedAt;
          const isPressed = state.pressedKeys.includes(key.name);
          if (isPressed && age >= stuckAfterMs) {
            stuckNames.push(key.name);
            return false;
          }
          return isPressed || age < state.lingerDurationMs;
        });
...
export const useKeyEvent = createKeyEventStore(
