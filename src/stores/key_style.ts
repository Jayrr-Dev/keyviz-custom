import {
  ...
  DEFAULT_HOTKEY_SETS,
  mergingDefaultHotkeys,
} from "@/lib/buildingDefaultHotkeys";
import { HotkeyLabel, HotkeyLabelSettings, HotkeySet } from "@/types/hotkey";
...
export type { HotkeyLabel, HotkeyLabelSettings, HotkeySet };

export const KEY_STYLE_STORE = "key_style_store";

export const KEY_PLACEMENT_CURSOR = "cursor" as const;
export const KEY_PLACEMENT_SCREEN = "screen" as const;

export interface AppearanceSettings {
  monitor: string | null;
  /** `cursor` follows the pointer. `screen` uses Alignment. */
  keyPlacement: "screen" | "cursor";
...
/**
 * True when the key bar should sit next to the pointer.
 */
export const pickingKeysFollowCursor = (
  appearance: Pick<AppearanceSettings, "keyPlacement">,
) => (appearance.keyPlacement ?? KEY_PLACEMENT_CURSOR) === KEY_PLACEMENT_CURSOR;
...
export interface KeyStyleState {
  ...
  mouse: MouseSettings;
  hotkeyLabels: HotkeyLabelSettings;
}
...
interface KeyStyleActions {
  ...
  setHotkeyLabels: (hotkeyLabels: Partial<HotkeyLabelSettings>) => void;
  import: () => Promise<void>;
  export: () => Promise<void>;
}
...
const createKeyStyleStore = createSyncedStore<KeyStyleStore>(
  ...
  (set, get) => ({
    ...
    hotkeyLabels: {
      ...
      activeSet: null,
      sets: DEFAULT_HOTKEY_SETS,
      labels: DEFAULT_HOTKEY_LABELS,
    },
    ...
    setAppearance: (appearance) =>
      set((state) => ({
        appearance: {
          ...state.appearance,
          keyPlacement: state.appearance.keyPlacement ?? KEY_PLACEMENT_CURSOR,
          ...appearance,
        },
      })),
    setLayout: (layout) =>
    ...
    setModifier: (modifier) =>
      set((state) => ({ modifier: { ...state.modifier, ...modifier } })),
    setText: (text) => set((state) => ({ text: { ...state.text, ...text } })),
    setBorder: (border) =>
      set((state) => ({ border: { ...state.border, ...border } })),
    setBackground: (background) =>
      set((state) => ({ background: { ...state.background, ...background } })),
    setMouse: (mouse) =>
      set((state) => ({ mouse: { ...state.mouse, ...mouse } })),
    setHotkeyLabels: (hotkeyLabels) =>
      set((state) => ({
        hotkeyLabels: { ...state.hotkeyLabels, ...hotkeyLabels },
      })),
    ...
    import: async () => {
      try {
        ...
        set(() => ({
          ...
          hotkeyLabels: parsedData.hotkeyLabels ?? {
            enabled: true,
            defaultEnabled: true,
            showDescription: false,
            activeSet: null,
    ...
    export: async () => {
      ...
      const exportData: KeyStyleState = {
        ...
        mouse: state.mouse,
        hotkeyLabels: state.hotkeyLabels,
      };
      try {
    ...
    persist(config, {
      ...
      merge: (persistedState, currentState) => {
        ...
        return {
          ...
          hotkeyLabels: (() => {
            const merged = mergingDefaultHotkeys(
              (persisted.hotkeyLabels?.sets ??
                currentState.hotkeyLabels.sets) as HotkeyLabelSettings["sets"],
              persisted.hotkeyLabels?.labels ??
                currentState.hotkeyLabels.labels,
            );
            return {
              ...currentState.hotkeyLabels,
              ...persisted.hotkeyLabels,
              defaultEnabled:
                persisted.hotkeyLabels?.defaultEnabled ??
                currentState.hotkeyLabels.defaultEnabled ??
                true,
              showDescription:
                persisted.hotkeyLabels?.showDescription ??
                currentState.hotkeyLabels.showDescription ??
                false,
              sets: merged.sets,
...
export const useKeyStyle = createKeyStyleStore(
